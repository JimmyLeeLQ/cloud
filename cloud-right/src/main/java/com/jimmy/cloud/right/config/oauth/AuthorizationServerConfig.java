package com.jimmy.cloud.right.config.oauth;

import com.jimmy.cloud.right.config.error.MssWebResponseExceptionTranslator;
...
public class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
    ...
    public void configure(ClientDetailsServiceConfigurer clients) throws Exception { clients.withClientDetails(clientDetails());
        ...
        //配置在内存中，也可以从数据库中获取
        String secret = new BCryptPasswordEncoder().encode("123");
        log.info("AuthorizationServerConfig  configure with secret:{}",secret);
        clients.inMemory() // 使用in-memory存储
                .withClient("jimmy") // client_id   android
                .scopes("read")
